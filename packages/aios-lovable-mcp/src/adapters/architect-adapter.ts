...
export class ArchitectAdapter {
  async execute(
  ...
  ): Promise<AgentResponse<DesignArchitectureOutput>> {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Unknown architect agent error';
      return {
        success: false,
        error: {
          code: 'AGENT_FAILURE',
  ...
  private async runAgent(
    input: DesignArchitectureInput
  ): Promise<DesignArchitectureOutput> {
    ...
    const frontend =
      stackPref === 'vite-react'
        ? 'Vite + React + TypeScript + Tailwind'
        : 'Next.js + React + TypeScript + Tailwind';

    const architecture: DesignArchitectureOutput = {
      architecture: {
        stack: {
          frontend,
