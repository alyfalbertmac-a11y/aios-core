...
export class UXAdapter {
  async execute(
    input: DesignUXInput
  ): Promise<AgentResponse<DesignUXOutput>> {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Unknown UX agent error';
      return {
        success: false,
        error: {
          code: 'AGENT_FAILURE',
  ...
  private async runAgent(input: DesignUXInput): Promise<DesignUXOutput> {
  ...
  private formatDesignMarkdown(system: any): string {
    return [
      '# Design System',
      '',
      '## Typography',
      system.typography.scales
        .map(
          (scale: any) =>
            `- **${scale.name}**: ${scale.size} (weight: ${scale.weight}, line-height: ${scale.line_height})`
        )
        .join('\n'),
      '',
      '## Colors',
      ...
      system.components
        .map(
          (comp: any) =>
            `### ${comp.name}\n${comp.variants?.map((v: any) => `- ${v.name}`).join('\n')}`
        )
        .join('\n'),
    ].join('\n');
  }

  private formatWireframesMarkdown(wireframes: any[]): string {
    return [
      '# Wireframes',
      '',
      ...wireframes.map(
        (wf) =>
          `## ${wf.page}\n**Layout**: ${wf.layout}\n\nKey Elements:\n${wf.key_elements.map((e: string) => `- ${e}`).join('\n')}`
      ),
    ].join('\n\n');
  }
