...
function createServer() {
    ...
    // -------------------------------------------------------------------------
    server.tool('aios_strategize', 'Generate comprehensive product strategy, market positioning, and PRD. Runs @pm agent (@analyst for market research).', {
    ...
    // Tool: aios_design_ux
    // -------------------------------------------------------------------------
    server.tool('aios_design_ux', 'Generate design system, component specs, wireframes, and accessibility guidelines. Runs @ux-design-expert agent.', {
        product_name: z.string().min(1).describe('Name of the product'),
        user_flows: z
            .array(z.string())
            .optional()
    ...
    // -------------------------------------------------------------------------
    server.tool('aios_full_pipeline', 'Execute end-to-end product development pipeline: strategy → design → architecture → code. Returns job_id for async tracking.', {
        product_name: z.string().min(1).describe('Name of the product'),
    ...
    // -------------------------------------------------------------------------
    server.tool('aios_get_status', 'Check the status and progress of an async job or pipeline execution.', {
    ...
    // -------------------------------------------------------------------------
    server.tool('aios_get_artifact', 'Retrieve generated artifacts from a completed job (PRD, strategy, design system, architecture, code).', {
...
async function main() {
    ...
    console.error(`[${SERVER_NAME}] Tools: aios_design_architecture, aios_generate_code, aios_strategize, aios_design_ux, aios_full_pipeline, aios_get_status, aios_get_artifact`);
