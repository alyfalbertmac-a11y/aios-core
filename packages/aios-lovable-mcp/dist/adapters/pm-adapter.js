...
export class PMAdapter {
    async execute(input) {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'Unknown PM agent error';
            return {
                success: false,
                error: {
                    code: 'AGENT_FAILURE',
    ...
    formatStrategyMarkdown(strategy) {
        return [
            '# Product Strategy',
            '',
            `## Vision`,
            ...
            strategy.target_segments
                .map((s) => `- **${s.segment}**: ${s.size} market, ${s.growth} growth`)
                .join('\n'),
            '',
            '## Positioning',
    ...
    formatPRDMarkdown(prd) {
        return [
            ...
            prd.functional_requirements
                .map((fr) => `### ${fr.id}: ${fr.requirement} (${fr.priority})\n${fr.category}`)
                .join('\n'),
            '',
            '## Non-Functional Requirements',
            prd.non_functional_requirements
                .map((nfr) => `- ${nfr.requirement} (${nfr.priority})`)
                .join('\n'),
            '',
            '## Success Metrics',
...
//# sourceMappingURL=pm-adapter.js.map